...
int main(int argc, char *argv[]){
    ...
    deleteOperations = deleteFraction*operationsCount;
 	...
 	//end = clock();
        ...
        clear_list(head_pp);
        //sampleTime[x] = (end-start)/CLOCKS_PER_SEC;
        sampleTime[x] = difftime(end, start);
    ...
    printf("Total time(s):%f (ms): %f\n",totalTime, totalTime*1000);
    printf("Average time(s):%f (ms): %f\n",avgTime, avgTime*1000);
    printf("Standerd Deviation(s):%f (ms): %f\n",stdDev, stdDev);
}
